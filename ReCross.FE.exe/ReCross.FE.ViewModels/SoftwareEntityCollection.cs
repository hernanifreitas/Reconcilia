...
public class SoftwareEntityCollection : ExtendedObservableCollection<ReCrossEntities, SoftwareEntity>
{
	public SoftwareEntityCollection()
	{
	}

	public SoftwareEntityCollection(IEnumerable<SoftwareEntity> entities)
		: base(entities)
	{
		string[] parentKeys = new string[1] { MenuItemEnum.Parametrization.ToString().ToLower() };
		string key = SubMenuItemEnum.SoftwareEntities.ToString().ToLower();
		foreach (SoftwareEntity entity in entities)
		{
	...
	protected override void InsertItem(int index, SoftwareEntity item)
	{
