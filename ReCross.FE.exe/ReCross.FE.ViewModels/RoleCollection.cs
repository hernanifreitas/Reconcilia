...
public class RoleCollection : ExtendedObservableCollection<ReCrossEntities, Role>
{
	public RoleCollection()
	{
	}

	public RoleCollection(IEnumerable<Role> entities)
		: base(entities)
	{
		string[] parentKeys = new string[1] { MenuItemEnum.Parametrization.ToString().ToLower() };
		string key = SubMenuItemEnum.Roles.ToString().ToLower();
	...
	{
		base.CurrentDataContext.AddToRoles(item);
