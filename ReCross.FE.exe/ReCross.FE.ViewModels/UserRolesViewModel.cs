...
public class UserRolesViewModel : ViewModelBase<ReCrossEntities, Role, UserRolesCollection>, IEntityRoles
{
	...
	protected override UserRolesCollection InitilizeDataCollection()
	{
	...
	public object SetUserRoles(IEnumerable<Role> roles)
	{
		if (!base.IsDataResetForbidden)
		{
			CollectionViewSource currentCVS = CurrentCVS;
			UserRolesCollection source = (base.CurrentDataCollection = ((roles == null) ? new UserRolesCollection(new List<Role>()) : new UserRolesCollection(roles)));
