...
public class BankAccountCollection : ExtendedObservableCollection<ReCrossEntities, BankAccount>
{
	public BankAccountCollection()
	{
	}

	public BankAccountCollection(IEnumerable<BankAccount> entities)
		: base(entities)
	{
		string[] parentKeys = new string[0];
		string key = MenuItemEnum.BankAccounts.ToString().ToLower();
		foreach (BankAccount entity in entities)
		{
	...
	protected override void InsertItem(int index, BankAccount item)
	{
		base.CurrentDataContext.AddToBankAccounts(item);
